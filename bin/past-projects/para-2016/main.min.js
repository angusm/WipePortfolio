!function(global, factory) {
    ...
    var dataSpace = ($.ui.position, "ui-effects-"), jQuery = $;
    $.effects = {
        effect: {}
...
function() {
    ...
    angular.module("para", [ "handies", "backendModels" ]);
}(), function() {
    angular.module("para2016", [ "para" ]);
}(), function() {
    ...
    function populateInstanceFactory(relationManager, caseTransform) {
        ...
        function hasPopulatableProperty(instance, propertyName) {
            ...
            for (var prototypeToCheck = instance.constructor.prototype; prototypeToCheck !== Object.prototype; ) {
    ...
    angular.module("backendModels").factory("populateInstance", [ "relationManager", "caseTransform", populateInstanceFactory ]);
}(), function() {
    ...
    function isRGBACapable() {
        var $script = $("script:first"), color = $script.css("color"), result = !1;
        if (/^rgba/.test(color)) result = !0; else try {
            result = color !== $script.css("color", "rgba(0, 0, 0, 0.5)").css("color"), $script.css("color", color);
        } catch (e) {}
        return $script.removeAttr("style"), result;
    }
    function calculateShadows(beginList, endList, pos) {
        var shadows = [];
        return $.each(beginList, function(i) {
            var parts = [], begin = beginList[i], end = endList[i];
            if (begin.inset && parts.push("inset"), "undefined" != typeof end.left && parts.push(parseFloat(begin.left + pos * (end.left - begin.left)) + "px " + parseFloat(begin.top + pos * (end.top - begin.top)) + "px"), 
            "undefined" != typeof end.blur && parts.push(parseFloat(begin.blur + pos * (end.blur - begin.blur)) + "px"), 
            "undefined" != typeof end.spread && parts.push(parseFloat(begin.spread + pos * (end.spread - begin.spread)) + "px"), 
            "undefined" != typeof end.color) {
                ...
                $.support.rgba && (color += "," + parseFloat(begin.color[3] + pos * (end.color[3] - begin.color[3]))), 
    ...
    function parseShadows(shadow) {
        function findInset() {
            ...
            return null !== m && m.length > 0 ? (parsedShadow.inset = !0, parsePosition += m[0].length, 
            !0) : !1;
        }
        function findOffsets() {
            var m = /^(-?[0-9\.]+)(?:px)?\s+(-?[0-9\.]+)(?:px)?(?:\s+(-?[0-9\.]+)(?:px)?)?(?:\s+(-?[0-9\.]+)(?:px)?)?/.exec(shadow.substring(parsePosition));
            return null !== m && m.length > 0 ? (parsedShadow.left = parseInt(m[1], 10), parsedShadow.top = parseInt(m[2], 10), 
            parsedShadow.blur = m[3] ? parseInt(m[3], 10) : 0, parsedShadow.spread = m[4] ? parseInt(m[4], 10) : 0, 
            parsePosition += m[0].length, !0) : !1;
        }
        function findColor() {
            var m = /^#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})/.exec(shadow.substring(parsePosition));
            return null !== m && m.length > 0 ? (parsedShadow.color = [ parseInt(m[1], 16), parseInt(m[2], 16), parseInt(m[3], 16), 1 ], 
            parsePosition += m[0].length, !0) : (m = /^#([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])/.exec(shadow.substring(parsePosition)), 
            null !== m && m.length > 0 ? (parsedShadow.color = [ 17 * parseInt(m[1], 16), 17 * parseInt(m[2], 16), 17 * parseInt(m[3], 16), 1 ], 
            parsePosition += m[0].length, !0) : (m = /^rgb\(\s*([0-9\.]+)\s*,\s*([0-9\.]+)\s*,\s*([0-9\.]+)\s*\)/.exec(shadow.substring(parsePosition)), 
            null !== m && m.length > 0 ? (parsedShadow.color = [ parseInt(m[1], 10), parseInt(m[2], 10), parseInt(m[3], 10), 1 ], 
            parsePosition += m[0].length, !0) : (m = /^rgba\(\s*([0-9\.]+)\s*,\s*([0-9\.]+)\s*,\s*([0-9\.]+)\s*,\s*([0-9\.]+)\s*\)/.exec(shadow.substring(parsePosition)), 
            null !== m && m.length > 0 ? (parsedShadow.color = [ parseInt(m[1], 10), parseInt(m[2], 10), parseInt(m[3], 10), parseFloat(m[4]) ], 
            parsePosition += m[0].length, !0) : !1)));
        }
        function findWhiteSpace() {
            var m = /^\s+/.exec(shadow.substring(parsePosition));
            return null !== m && m.length > 0 ? (parsePosition += m[0].length, !0) : !1;
        }
        function findComma() {
        ...
        function normalizeShadow(shadow) {
            if ($.isPlainObject(shadow)) {
                ...
                for ($.isArray(shadow.color) && (sColor = shadow.color, cLength = sColor.length), 
                i = 0; 4 > i; i++) cLength > i ? color.push(sColor[i]) : 3 === i ? color.push(1) : color.push(0);
            }
            return $.extend({
                left: 0,
                top: 0,
        ...
        for (var parsedShadows = [], parsePosition = 0, parseLength = shadow.length, parsedShadow = normalizeShadow(); parseLength > parsePosition; ) if (findInset()) findWhiteSpace(); else if (findOffsets()) findWhiteSpace(); else if (findColor()) findWhiteSpace(); else {
            if (!findComma()) break;
            parsedShadows.push(normalizeShadow(parsedShadow)), parsedShadow = {};
        }
        return parsedShadows.push(normalizeShadow(parsedShadow)), parsedShadows;
    }
    $.extend(!0, $, {
        support: {
            rgba: isRGBACapable()
    ...
    var boxShadowProperty, styles = $("html").prop("style");
    $.each([ "boxShadow", "MozBoxShadow", "WebkitBoxShadow" ], function(i, property) {
        var val = styles[property];
        return "undefined" != typeof val ? (boxShadowProperty = property, !1) : void 0;
    }), boxShadowProperty && ($.Tween.propHooks.boxShadow = {
        get: function(tween) {
            return $(tween.elem).css(boxShadowProperty);
        },
        set: function(tween) {
            var i, style = tween.elem.style, p_begin = parseShadows($(tween.elem)[0].style[boxShadowProperty] || $(tween.elem).css(boxShadowProperty)), p_end = parseShadows(tween.end), maxShadowCount = Math.max(p_begin.length, p_end.length);
            for (i = 0; maxShadowCount > i; i++) p_end[i] = $.extend({}, p_begin[i], p_end[i]), 
            p_begin[i] ? "color" in p_begin[i] && $.isArray(p_begin[i].color) !== !1 || (p_begin[i].color = p_end[i].color || [ 0, 0, 0, 0 ]) : p_begin[i] = parseShadows("0 0 0 0 rgba(0,0,0,0)")[0];
    ...
    function roomPhoto() {
        var directive = {
            ...
            templateUrl: "templates/room_photo.html",
            controller: [ "$scope", "$element", "PositionableElement", "randFade", controller ]
        };
        return directive;
    }
    function controller($scope, $element, PositionableElement, randFade) {
        function applyStyle() {
            $element.css(getStyle());
        }
        function attachEventListeners() {
            getRoomPhoto().on(PositionableElement.EVENTS.SHOW, function(event) {
                randFade["in"]($element, MIN_TRANS_TIME, MAX_TRANS_TIME);
            }), getRoomPhoto().on(PositionableElement.EVENTS.HIDE, function(event) {
                randFade.out($element, MIN_TRANS_TIME, MAX_TRANS_TIME);
            });
        }
        function getStyle() {
            var style = getRoomPhoto().getPercentAbsolutePositionedCSS();
            return style.background = "url(" + getRoomPhoto().src + ")", style["background-size"] = "100% 100%", 
            style;
        }
        function getRoomPhoto() {
            return $scope.roomPhoto;
        }
        applyStyle(), attachEventListeners();
        var MIN_TRANS_TIME = 300, MAX_TRANS_TIME = 750;
    }
    angular.module("para").directive("roomPhoto", roomPhoto);
}(), function() {
    function stockPhoto() {
        var directive = {
            restrict: "E",
            ...
            templateUrl: "templates/stock_photo.html",
            controller: [ "$scope", "$element", "PositionableElement", "randFade", controller ]
        };
        return directive;
    }
    function controller($scope, $element, PositionableElement, randFade) {
        ...
        function attachEventListeners() {
            getStockPhoto().on(PositionableElement.EVENTS.SHOW, function(event) {
                randFade["in"]($element, MIN_TRANS_TIME, MAX_TRANS_TIME);
            }), getStockPhoto().on(PositionableElement.EVENTS.HIDE, function(event) {
                randFade.out($element, MIN_TRANS_TIME, MAX_TRANS_TIME);
            });
        }
        function getStyle() {
            var style = getStockPhoto().getPercentAbsolutePositionedCSS();
            return style.background = "url(" + getStockPhoto().src + ")", style["background-size"] = "100% 100%", 
            style;
        }
        function getStockPhoto() {
            return $scope.stockPhoto;
        }
        applyStyle(), attachEventListeners();
        var MIN_TRANS_TIME = 300, MAX_TRANS_TIME = 750;
    }
    angular.module("para").directive("stockPhoto", stockPhoto);
}(), function() {
    function swatch() {
        var directive = {
            restrict: "E",
            ...
            templateUrl: "templates/swatch.html",
            controller: [ "$scope", "$element", "Size", "PositionableElement", "randFade", controller ]
        };
        return directive;
    }
    function controller($scope, $element, Size, PositionableElement, randFade) {
        function mouseIn() {
            getSwatch().setScale(new Size(SCALE_ON_HOVER, SCALE_ON_HOVER));
            var style = getStyle();
            style["box-shadow"] = "1px 1px 1px 0px rgba(50, 50, 50, 0.75)", $element.animate(style, {
                duration: ANIM_TIME,
                queue: !1
            }), $element.find(".name").fadeIn({
                duration: ANIM_TIME,
                queue: !1
            }), $element.find(".code").fadeOut({
                duration: ANIM_TIME,
                queue: !1
            });
        }
        function mouseOut() {
            getSwatch().setScale(new Size(1, 1));
            var style = getStyle();
            $element.animate(style, {
                duration: ANIM_TIME,
                queue: !1
            }), $element.find(".name").fadeOut({
                duration: ANIM_TIME,
                queue: !1
            }), $element.find(".code").fadeIn({
                duration: ANIM_TIME,
                queue: !1
            });
        }
        function attachEventListeners() {
            $element.on("mouseenter", mouseIn), $element.on("mouseleave", mouseOut), getSwatch().on(PositionableElement.EVENTS.SHOW, function(event) {
                randFade["in"]($element, MIN_TRANS_TIME, MAX_TRANS_TIME);
            }), getSwatch().on(PositionableElement.EVENTS.HIDE, function(event) {
                randFade.out($element, MIN_TRANS_TIME, MAX_TRANS_TIME);
            });
        }
        function applyStyle() {
        ...
        function getStyle() {
            var style = getSwatch().getPercentAbsolutePositionedCSS();
            return style["box-shadow"] = "0px 0px 1px 0px rgba(50, 50, 50, 0.75)", style;
        }
        function getFontColourCSSClass() {
            return getSwatch().isDark ? "light-font" : "dark-font";
        }
        function getColourBlockStyle() {
            return "undefined" == typeof getSwatch().src ? {
                "background-color": getSwatch().colour.getCSSHexadecimalValue()
            } : {
                "background-image": "url(" + getSwatch().src + ")",
        ...
        $scope.getColourBlockStyle = getColourBlockStyle, $scope.getFontColourCSSClass = getFontColourCSSClass, 
        $element.find(".name").hide(), attachEventListeners(), applyStyle();
        var ANIM_TIME = 150, SCALE_ON_HOVER = 1.05, MIN_TRANS_TIME = 300, MAX_TRANS_TIME = 750;
    }
    angular.module("para").directive("swatch", swatch);
}(), function() {
    function trendButton() {
        var directive = {
            ...
            templateUrl: "templates/trend_button.html",
            controller: [ "$scope", "$element", "PositionableElement", "Size", trendButtonController ]
        };
        return directive;
    }
    function trendButtonController($scope, $element, PositionableElement, Size) {
        function mouseOver() {
            imageElement.setScale(new Size(SCALE_ON_HOVER, SCALE_ON_HOVER));
            var style = getImageStyle();
            style["box-shadow"] = "2px 2px 2px 0px rgba(50, 50, 50, 0.75)", updateImage(style);
        }
        function updateImage(style) {
            $element.find(".trend-button-image").animate(style, {
                duration: ANIM_TIME,
                queue: !1
            });
        }
        function mouseOut() {
            imageElement.setScale(new Size(1, 1)), updateImage(getImageStyle());
        }
        function applyListeners() {
            $element.on("mouseover", mouseOver), $element.on("mouseout", mouseOut);
        }
        function getImageStyle() {
            var style = imageElement.getPercentAbsolutePositionedCSS();
            return style.background = "url(" + getTrendGroup().roomPhotos[0].src + ")", style["box-shadow"] = "0px 0px 1px 0px rgba(50, 50, 50, 0.75)", 
            style["background-size"] = "cover", style["background-position"] = "fit", style;
        }
        function getTitle() {
            return getTrendGroup().title;
        }
        function getTrendGroup() {
            return $scope.trend;
        }
        $scope.getImageStyle = getImageStyle, $scope.getTitle = getTitle;
        var imageElement = new PositionableElement(0, 0, 100, 100);
        applyListeners();
        var ANIM_TIME = 150, SCALE_ON_HOVER = 1.05;
    }
    angular.module("para").directive("trendButton", trendButton);
}(), function() {
    function trendGroup() {
        function controller($scope, $element, PositionableElement, randFade) {
            function attachEventListeners() {
                getTrend().on(PositionableElement.EVENTS.SHOW, function(event) {
                    ele.css("pointer-events", "all"), randFade["in"](getBlurbElement(), MIN_TRANS_TIME, MAX_TRANS_TIME);
                }), getTrend().on(PositionableElement.EVENTS.HIDE, function(event) {
                    ele.css("pointer-events", "none"), randFade.out(getBlurbElement(), MIN_TRANS_TIME, MAX_TRANS_TIME);
                });
            }
            function getBlurbStyle() {
                return getTrend().blurbElement.getPercentAbsolutePositionedCSS();
            }
            function getBlurbElement() {
                return $element.find(".blurb");
            }
            function getTrend() {
                return $scope.trend;
            }
            $scope.getBlurbStyle = getBlurbStyle;
            var MIN_TRANS_TIME = 300, MAX_TRANS_TIME = 750, ele = $element;
        ...
        var directive = {
            ...
            templateUrl: "templates/trend_group.html",
            controller: [ "$scope", "$element", "PositionableElement", "randFade", controller ]
        };
        return directive;
    }
    angular.module("para").directive("trendGroup", trendGroup);
}(), function() {
    function ColourFactory(BaseModel, extend, hexTools) {
        function Colour(red, green, blue) {
            this.red = red, this.green = green, this.blue = blue, BaseModel.apply(this, arguments);
        }
        function getHexadecimalValue() {
            return hexTools.numberToPaddedHex(this.red, 2) + hexTools.numberToPaddedHex(this.green, 2) + hexTools.numberToPaddedHex(this.blue, 2);
        }
        function getCSSHexadecimalValue() {
            return "#" + this.getHexadecimalValue();
        }
        return extend(Colour, BaseModel), Colour.prototype.red = void 0, Colour.prototype.green = void 0, 
        Colour.prototype.blue = void 0, Colour.prototype.getHexadecimalValue = getHexadecimalValue, 
        Colour.prototype.getCSSHexadecimalValue = getCSSHexadecimalValue, Colour;
    }
    angular.module("para").factory("Colour", [ "BaseModel", "extend", "hexTools", ColourFactory ]);
}(), function() {
    function PositionFactory(BaseModel, extend, getFirstDefined) {
        function Position(x, y) {
            this.x = getFirstDefined(x, 0), this.y = getFirstDefined(y, 0), BaseModel.apply(this, arguments);
        }
        return extend(Position, BaseModel), Position.prototype.x = void 0, Position.prototype.y = void 0, 
        Position;
    }
    angular.module("para").factory("Position", [ "BaseModel", "extend", "getFirstDefined", PositionFactory ]);
}(), function() {
    function PositionableElementFactory(BaseModel, extend, relationManager, Position, Transition, Size, getFirstDefined) {
        function PositionableElement(x, y, width, height) {
            this.visible = !0, this.position = new Position(getFirstDefined(x, 0), getFirstDefined(y, 0)), 
            this.size = new Size(getFirstDefined(width, 0), getFirstDefined(height, 0)), this.__translation = new Position(0, 0), 
            this.__scale = new Size(1, 1), setupEventHandler(this), BaseModel.apply(this, arguments);
        }
        function setupEventHandler(instance) {
            instance.__eventCallbacks = {}, Object.keys(PositionableElement.EVENTS).forEach(function(eventString) {
                var internalString = PositionableElement.EVENTS[eventString];
                instance.__eventCallbacks[internalString] = [];
            });
        }
        function on(eventString, callback) {
            this.__eventCallbacks[eventString].push(callback);
        }
        function emit(eventString) {
            var event = {
                target: this
            };
            this.__eventCallbacks[eventString].forEach(function(callback) {
                return callback(event);
            });
        }
        function show() {
            this.emit(PositionableElement.EVENTS.SHOW);
        }
        function hide() {
            this.emit(PositionableElement.EVENTS.HIDE);
        }
        function getAbsolutePositionedCSS(containerWidth, containerHeight, unit) {
            return {
                position: "absolute",
                left: this.getLeftValue(containerWidth) + "" + unit,
                right: this.getRightValue(containerWidth) + "" + unit,
                bottom: this.getBottomValue(containerHeight) + "" + unit,
                top: this.getTopValue(containerHeight) + "" + unit,
                width: this.getWidth() + "" + unit,
                height: this.getHeight() + "" + unit
            };
        }
        function getPercentAbsolutePositionedCSS() {
            return this.getAbsolutePositionedCSS(100, 100, "%");
        }
        function getWidth() {
            return this.getRawWidth() * this.getScale().width;
        }
        ...
        function scale(scale) {
            this.setScale(this.getScale().width * scale.width, this.getScale().height * scale.height);
        }
        function setScale(scale) {
            this.__scale = scale;
        }
        function getHeight() {
            return this.getRawHeight() * this.getScale().height;
        }
        function getRawHeight() {
            return this.getSize().height;
        }
        function getWidthScaleDifference() {
            return this.getWidth() - this.getRawWidth();
        }
        function getHeightScaleDifference() {
            return this.getHeight() - this.getRawHeight();
        }
        function getHorizontalValueAdjustment() {
            return this.getWidthScaleDifference() / 2;
        }
        function getVerticalValueAdjustment() {
            return this.getHeightScaleDifference() / 2;
        }
        function getLeftValue(containerWidth) {
            return getValueFromOriginPoint(this.position.x, this.getSize().width, containerWidth) - this.getHorizontalValueAdjustment();
        }
        function getRightValue(containerWidth) {
            return getValueFromEndPoint(this.position.x, this.getSize().width, containerWidth) - this.getHorizontalValueAdjustment();
        }
        function getTopValue(containerHeight) {
            return getValueFromOriginPoint(this.position.y, this.getSize().height, containerHeight) - this.getVerticalValueAdjustment();
        }
        function getBottomValue(containerHeight) {
            return getValueFromEndPoint(this.position.y, this.getSize().height, containerHeight) - this.getVerticalValueAdjustment();
        }
        function getValueFromEndPoint(rawValue, sizeAdjustment, containerSize) {
            return isPositive(rawValue) ? containerSize - (rawValue + sizeAdjustment) : Math.abs(rawValue);
        }
        function getValueFromOriginPoint(rawValue, sizeAdjustment, containerSize) {
            return isPositive(rawValue) ? rawValue : containerSize - (Math.abs(rawValue) + sizeAdjustment);
        }
        function isPositive(value) {
            return 1 == Math.sign(1 / value);
        }
        return extend(PositionableElement, BaseModel), PositionableElement.EVENTS = {}, 
        PositionableElement.EVENTS.SHOW = "Show", PositionableElement.EVENTS.HIDE = "Hide", 
        PositionableElement.prototype.position = void 0, PositionableElement.prototype.size = void 0, 
        PositionableElement.prototype.visible = void 0, PositionableElement.prototype.__translation = void 0, 
        PositionableElement.prototype.__scale = void 0, PositionableElement.prototype.__eventCallbacks = void 0, 
        relationManager.registerHasOneRelation(PositionableElement, "position", Position), 
        relationManager.registerHasOneRelation(PositionableElement, "size", Size), relationManager.registerHasOneRelation(PositionableElement, "transition", Transition), 
        PositionableElement.prototype.getLeftValue = getLeftValue, PositionableElement.prototype.getRightValue = getRightValue, 
        PositionableElement.prototype.getBottomValue = getBottomValue, PositionableElement.prototype.getTopValue = getTopValue, 
        PositionableElement.prototype.getRawWidth = getRawWidth, PositionableElement.prototype.getRawHeight = getRawHeight, 
        PositionableElement.prototype.getWidth = getWidth, PositionableElement.prototype.getHeight = getHeight, 
        PositionableElement.prototype.getAbsolutePositionedCSS = getAbsolutePositionedCSS, 
        PositionableElement.prototype.getPercentAbsolutePositionedCSS = getPercentAbsolutePositionedCSS, 
        PositionableElement.prototype.scale = scale, PositionableElement.prototype.setScale = setScale, 
        PositionableElement.prototype.getScale = getScale, PositionableElement.prototype.getSize = getSize, 
        PositionableElement.prototype.getWidthScaleDifference = getWidthScaleDifference, 
        PositionableElement.prototype.getHeightScaleDifference = getHeightScaleDifference, 
        PositionableElement.prototype.getHorizontalValueAdjustment = getHorizontalValueAdjustment, 
        PositionableElement.prototype.getVerticalValueAdjustment = getVerticalValueAdjustment, 
        PositionableElement.prototype.on = on, PositionableElement.prototype.emit = emit, 
        PositionableElement.prototype.show = show, PositionableElement.prototype.hide = hide, 
        PositionableElement;
    }
    angular.module("para").factory("PositionableElement", [ "BaseModel", "extend", "relationManager", "Position", "Transition", "Size", "getFirstDefined", PositionableElementFactory ]);
}(), function() {
    function RoomPhotoFactory(PositionableElement, extend) {
        function RoomPhoto() {
            PositionableElement.apply(this, arguments);
        }
        return extend(RoomPhoto, PositionableElement), RoomPhoto.prototype.src = void 0, 
        RoomPhoto;
    }
    angular.module("para").factory("RoomPhoto", [ "PositionableElement", "extend", RoomPhotoFactory ]);
}(), function() {
    function SizeFactory(BaseModel, extend, getFirstDefined) {
        function Size(width, height) {
            this.width = getFirstDefined(width, 0), this.height = getFirstDefined(height, 0), 
            BaseModel.apply(this, arguments);
        }
        return extend(Size, BaseModel), Size.prototype.width = void 0, Size.prototype.height = void 0, 
        Size;
    }
    angular.module("para").factory("Size", [ "BaseModel", "extend", "getFirstDefined", SizeFactory ]);
}(), function() {
    function StockPhotoFactory(PositionableElement, extend) {
        function StockPhoto() {
            PositionableElement.apply(this, arguments);
        }
        return extend(StockPhoto, PositionableElement), StockPhoto.prototype.src = void 0, 
        StockPhoto;
    }
    angular.module("para").factory("StockPhoto", [ "PositionableElement", "extend", StockPhotoFactory ]);
}(), function() {
    function SwatchFactory(PositionableElement, relationManager, extend, Colour) {
        function Swatch() {
            this.isDark = !1, PositionableElement.apply(this, arguments);
        }
        return extend(Swatch, PositionableElement), Swatch.prototype.name = void 0, Swatch.prototype.code = void 0, 
        Swatch.prototype.colour = void 0, Swatch.prototype.src = void 0, Swatch.prototype.isDark = void 0, 
        relationManager.registerHasOneRelation(Swatch, "colour", Colour), Swatch;
    }
    angular.module("para").factory("Swatch", [ "PositionableElement", "relationManager", "extend", "Colour", SwatchFactory ]);
}(), function() {
    function TransitionFactory(BaseModel, extend) {
        function Transition() {
            BaseModel.apply(this, arguments);
        }
        return extend(Transition, BaseModel), Transition;
    }
    angular.module("para").factory("Transition", [ "BaseModel", "extend", TransitionFactory ]);
}(), function() {
    function TrendGroupFactory(extend, relationManager, Swatch, StockPhoto, RoomPhoto, PositionableElement) {
        function TrendGroup() {
            PositionableElement.apply(this, arguments);
            var _this = this;
            this.on(PositionableElement.EVENTS.SHOW, function(event) {
                _this.swatches.forEach(function(swatch) {
                    swatch.show();
                }), _this.stockPhotos.forEach(function(stockPhoto) {
                    stockPhoto.show();
                }), _this.roomPhotos.forEach(function(roomPhoto) {
                    roomPhoto.show();
                });
            }), this.on(PositionableElement.EVENTS.HIDE, function(event) {
                _this.swatches.forEach(function(swatch) {
                    swatch.hide();
                }), _this.stockPhotos.forEach(function(stockPhoto) {
                    stockPhoto.hide();
        ...
        return extend(TrendGroup, PositionableElement), TrendGroup.prototype.title = void 0, 
        TrendGroup.prototype.description = void 0, TrendGroup.prototype.swatches = void 0, 
        TrendGroup.prototype.stockPhotos = void 0, TrendGroup.prototype.roomPhotos = void 0, 
        relationManager.registerHasManyRelation(TrendGroup, "swatches", Swatch), relationManager.registerHasManyRelation(TrendGroup, "stockPhotos", StockPhoto), 
        relationManager.registerHasManyRelation(TrendGroup, "roomPhotos", RoomPhoto), TrendGroup;
    }
    angular.module("para").factory("TrendGroup", [ "extend", "relationManager", "Swatch", "StockPhoto", "RoomPhoto", "PositionableElement", TrendGroupFactory ]);
}(), function() {
    function randFadeFactory(getRandom) {
        function randFadeIn(element, minTime, maxTime) {
            randFade(element, "fadeIn", minTime, maxTime);
        }
        function randFade(element, funcToCall, minTime, maxTime) {
            var delay = getRandom(0, minTime), animMinTime = minTime - delay, animMaxTime = maxTime - delay;
            setTimeout(function() {
                element[funcToCall](getRandom(animMinTime, animMaxTime));
            }, delay);
        }
        function randFadeOut(element, minTime, maxTime) {
            randFade(element, "fadeOut", minTime, maxTime);
        }
        return {
            out: randFadeOut,
            "in": randFadeIn
    ...
    angular.module("para").factory("randFade", [ "getRandom", randFadeFactory ]);
}(), function() {
    function ArtisanTrendGroupFactory(TrendGroup2016, extend, createPopulatedInstances, createPopulatedInstance, Colour, StockPhoto, RoomPhoto, SmallSwatch, LargeSwatch, Position, Size, PositionableElement) {
        function ArtisanTrendGroup() {
            this.title = "artisan", this.description = "Coming in off the grid with passion for authentic one-of-a-kind craft and artistry. A labour of love for colours, finishes, materials and textures that inspire creativity in an era where working from home is a growing trend.";
            var smallSwatches = createPopulatedInstances(SmallSwatch, [ {
                name: "cutting edge",
                code: "P5220-72",
                colour: new Colour(99, 94, 89),
                isDark: !0,
                position: new Position(0, 0)
            }, {
                name: "elegant boutique",
                code: "P5220-51D",
                colour: new Colour(139, 138, 141),
                isDark: !0,
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, 0)
            }, {
                name: "return to the gallows",
                code: "P5216-41D",
                colour: new Colour(195, 181, 140),
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 0)
            }, {
                name: "spring sage",
                code: "PF40",
                colour: new Colour(197, 198, 193),
                position: new Position(0, SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)
            }, {
                name: "singapore coral",
                code: "P5077-63",
                colour: new Colour(218, 117, 77),
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)
            }, {
                name: "pickled oak",
                code: "Minwax",
                src: "images/artisan/minwax.jpg",
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)
            }, {
                name: "copper",
                code: "Krylon",
                src: "images/artisan/krylon.jpg",
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            } ]), largeSwatches = createPopulatedInstances(LargeSwatch, [ {
                name: "dazzle denim",
                code: "PF72",
                colour: new Colour(57, 81, 115),
                isDark: !0,
                position: new Position(2 * -(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -0)
            }, {
                name: "white chocolate",
                code: "PF27",
                colour: new Colour(240, 232, 215),
                position: new Position(-LargeSwatch.SIZE_PERCENT - TrendGroup2016.GAP_PERCENT, -0)
            }, {
                name: "her majesty",
                code: "P5149-85",
                colour: new Colour(77, 48, 64),
                isDark: !0,
                position: new Position(-0, -0)
            } ]);
            this.swatches = [].concat(smallSwatches, largeSwatches), this.stockPhotos = createPopulatedInstances(StockPhoto, [ {
                src: "images/artisan/stock.jpg",
                position: new Position(0, -0),
                size: new Size(100 - 3 * (LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), LargeSwatch.SIZE_PERCENT)
            } ]), this.roomPhotos = createPopulatedInstances(RoomPhoto, [ {
                src: "images/artisan/room.jpg",
                position: new Position(-0, 0),
                size: new Size(100 - 3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), TrendGroup2016.ROOM_VERTICAL)
            } ]), this.blurbElement = createPopulatedInstance(PositionableElement, {
                position: new Position(0, 3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)),
                size: new Size(3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), TrendGroup2016.ROOM_VERTICAL - 3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }), TrendGroup2016.apply(this, arguments);
        }
        return extend(ArtisanTrendGroup, TrendGroup2016), ArtisanTrendGroup;
    }
    angular.module("para2016").factory("ArtisanTrendGroup", [ "TrendGroup2016", "extend", "createPopulatedInstances", "createPopulatedInstance", "Colour", "StockPhoto", "RoomPhoto", "SmallSwatch", "LargeSwatch", "Position", "Size", "PositionableElement", ArtisanTrendGroupFactory ]);
}(), function() {
    function para2016Controller($scope, ArtisanTrendGroup, DiscoveryTrendGroup, PassionTrendGroup, UrbanJungleTrendGroup, $timeout) {
        function scaleTrendVerticals() {
            var VERTICAL_SCALE = 100 / HEIGHT_OVER_WIDTH;
            getTrends().forEach(function(trend) {
                trend.swatches.forEach(function(swatch) {
                    swatch.position.y *= VERTICAL_SCALE, swatch.size.height *= VERTICAL_SCALE;
                }), trend.stockPhotos.forEach(function(stockPhoto) {
                    stockPhoto.position.y *= VERTICAL_SCALE, stockPhoto.size.height *= VERTICAL_SCALE;
                }), trend.roomPhotos.forEach(function(roomPhoto) {
                    roomPhoto.position.y *= VERTICAL_SCALE, roomPhoto.size.height *= VERTICAL_SCALE;
                }), trend.blurbElement.position.y *= VERTICAL_SCALE, trend.blurbElement.size.height *= VERTICAL_SCALE;
            });
        }
        function getTrends() {
            return $scope.trends;
        }
        function selectTrend(trendToSelect) {
            getTrends().forEach(function(trend) {
                trend !== trendToSelect && trend.hide();
            }), setTimeout(function() {
                trendToSelect.show();
            }, ANIM_TIME), $scope.selectedTrend = trendToSelect;
        }
        function getSelectedTrend() {
            return $scope.selectedTrend;
        }
        var HEIGHT_OVER_WIDTH = 77.5;
        $scope.trends = [ new PassionTrendGroup(), new ArtisanTrendGroup(), new UrbanJungleTrendGroup(), new DiscoveryTrendGroup() ], 
        $scope.selectedTrend = $scope.trends[0], $scope.getSelectedTrend = getSelectedTrend, 
        $scope.selectTrend = selectTrend, scaleTrendVerticals(), $timeout(function() {
            selectTrend($scope.trends[0]);
        }, 500);
        var ANIM_TIME = 300;
    }
    angular.module("para2016").controller("para2016Controller", [ "$scope", "ArtisanTrendGroup", "DiscoveryTrendGroup", "PassionTrendGroup", "UrbanJungleTrendGroup", "$timeout", para2016Controller ]);
}(), function() {
    function DiscoveryTrendGroupFactory(TrendGroup2016, extend, createPopulatedInstances, createPopulatedInstance, SmallSwatch, LargeSwatch, Colour, StockPhoto, RoomPhoto, Position, Size, PositionableElement) {
        function DiscoveryTrendGroup() {
            this.title = "discovery", this.description = "Inspired by the unchartered domains of sea and space. Discovering fluid, luminescent and translucent colours and implementing them into our  own living spaces.";
            var smallSwatches = createPopulatedInstances(SmallSwatch, [ {
                name: "melon balls",
                code: "P5072-34",
                colour: new Colour(253, 227, 216),
                position: new Position(0, -(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                name: "butterfly jasmine",
                code: "PF26",
                colour: new Colour(244, 242, 241),
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, -(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                name: "white wash pickling",
                code: "Minwax",
                src: "images/discovery/minwax.jpg",
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                name: "aqua",
                code: "Krylon",
                src: "images/discovery/krylon.jpg",
                position: new Position(3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                name: "jazzleberry jam",
                code: "PF16",
                colour: new Colour(217, 67, 105),
                position: new Position(0, -0)
            }, {
                name: "egyptian blue",
                code: "P5009-63D",
                colour: new Colour(0, 130, 187),
                isDark: !0,
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, -0)
            }, {
                name: "candy floss",
                code: "P5042-34D",
                colour: new Colour(226, 237, 193),
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -0)
            }, {
                name: "sea foam",
                code: "Krylon",
                src: "images/discovery/krylon2.jpg",
                position: new Position(3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -0)
            } ]), largeSwatches = createPopulatedInstances(LargeSwatch, [ {
                name: "orchid mist",
                code: "PF5",
                colour: new Colour(243, 239, 247),
                position: new Position(2 * -(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 0)
            }, {
                name: "cobalt ice",
                code: "PF67",
                colour: new Colour(71, 130, 179),
                isDark: !0,
                position: new Position(-(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 0)
            }, {
                name: "trinidad and tobago",
                code: "P5010-85",
                colour: new Colour(25, 44, 71),
                isDark: !0,
                position: new Position(-0, 0)
            } ]);
            this.swatches = [].concat(smallSwatches, largeSwatches), this.stockPhotos = createPopulatedInstances(StockPhoto, [ {
                src: "images/discovery/stock_top.jpg",
                position: new Position(0, 0),
                size: new Size(100 - 3 * (LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), LargeSwatch.SIZE_PERCENT)
            }, {
                src: "images/discovery/stock_left.jpg",
                position: new Position(0, LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT),
                size: new Size(2 * SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, TrendGroup2016.ROOM_VERTICAL - 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            } ]), this.roomPhotos = createPopulatedInstances(RoomPhoto, [ {
                src: "images/discovery/room.jpg",
                position: new Position(-0, -0),
                size: new Size(100 - 4 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), TrendGroup2016.ROOM_VERTICAL)
            } ]), this.blurbElement = createPopulatedInstance(PositionableElement, {
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT),
                size: new Size(2 * SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, TrendGroup2016.ROOM_VERTICAL - 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }), TrendGroup2016.apply(this, arguments);
        }
        return extend(DiscoveryTrendGroup, TrendGroup2016), DiscoveryTrendGroup;
    }
    angular.module("para2016").factory("DiscoveryTrendGroup", [ "TrendGroup2016", "extend", "createPopulatedInstances", "createPopulatedInstance", "SmallSwatch", "LargeSwatch", "Colour", "StockPhoto", "RoomPhoto", "Position", "Size", "PositionableElement", DiscoveryTrendGroupFactory ]);
}(), function() {
    function LargeSwatchFactory(Swatch, extend, Size, Position) {
        function LargeSwatch() {
            Swatch.apply(this, arguments), this.size = new Size(LargeSwatch.SIZE_PERCENT, LargeSwatch.SIZE_PERCENT), 
            this.position = new Position(0, 0);
        }
        return extend(LargeSwatch, Swatch), LargeSwatch.SIZE_PERCENT = 21.25, LargeSwatch;
    }
    angular.module("para2016").factory("LargeSwatch", [ "Swatch", "extend", "Size", "Position", LargeSwatchFactory ]);
}(), function() {
    function PassionTrendGroupFactory(TrendGroup2016, extend, createPopulatedInstances, createPopulatedInstance, SmallSwatch, LargeSwatch, Colour, StockPhoto, RoomPhoto, Position, Size, PositionableElement) {
        function PassionTrendGroup() {
            this.title = "passion", this.description = "Spice it up! Evoke your basic human senses with colours and finishes that fire up and motivate you  to create with the utmost passion. A design recipe  to be passed on from generation to generation.";
            var smallSwatches = createPopulatedInstances(SmallSwatch, [ {
                name: "18kt gold plate",
                code: "Krylon",
                src: "images/passion/krylon.jpg",
                position: new Position(0, -(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                name: "glorious gladiolas",
                code: "P5045-52",
                colour: new Colour(173, 183, 123),
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, -(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                name: "crysler hall white",
                code: "P2118-00",
                colour: new Colour(247, 245, 244),
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                name: "dark walnut",
                code: "Minwax",
                src: "images/passion/minwax.jpg",
                isDark: !0,
                position: new Position(0, -0)
            }, {
                name: "mulling spice",
                code: "PF21",
                colour: new Colour(84, 61, 58),
                isDark: !0,
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, -0)
            }, {
                name: "saguenay tint 2",
                code: "PF2101-01",
                colour: new Colour(112, 124, 124),
                isDark: !0,
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -0)
            } ]), largeSwatches = createPopulatedInstances(LargeSwatch, [ {
                name: "cherries jubilee",
                code: "P5079-85",
                colour: new Colour(158, 28, 32),
                isDark: !0,
                position: new Position(2 * -(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 0)
            }, {
                name: "new romantic",
                code: "P5129-14",
                colour: new Colour(255, 246, 237),
                position: new Position(-(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 0)
            }, {
                name: "memory overload",
                code: "P5222-34D",
                colour: new Colour(210, 200, 189),
                position: new Position(-0, 0)
            } ]);
            this.swatches = [].concat(smallSwatches, largeSwatches), this.stockPhotos = createPopulatedInstances(StockPhoto, [ {
                src: "images/passion/stock.jpg",
                position: new Position(0, LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT),
                size: new Size(3 * SmallSwatch.SIZE_PERCENT + 2 * TrendGroup2016.GAP_PERCENT, TrendGroup2016.ROOM_VERTICAL - 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            } ]), this.roomPhotos = createPopulatedInstances(RoomPhoto, [ {
                src: "images/passion/room.jpg",
                position: new Position(-0, LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT),
                size: new Size(100 - 3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), TrendGroup2016.ROOM_VERTICAL)
            } ]), this.blurbElement = createPopulatedInstance(PositionableElement, {
                position: new Position(0, 0),
                size: new Size(3 * SmallSwatch.SIZE_PERCENT + 2 * TrendGroup2016.GAP_PERCENT, LargeSwatch.SIZE_PERCENT)
            }), TrendGroup2016.apply(this, arguments);
        }
        return extend(PassionTrendGroup, TrendGroup2016), PassionTrendGroup;
    }
    angular.module("para2016").factory("PassionTrendGroup", [ "TrendGroup2016", "extend", "createPopulatedInstances", "createPopulatedInstance", "SmallSwatch", "LargeSwatch", "Colour", "StockPhoto", "RoomPhoto", "Position", "Size", "PositionableElement", PassionTrendGroupFactory ]);
}(), function() {
    function SmallSwatchFactory(Swatch, extend, Size, Position) {
        function SmallSwatch() {
            Swatch.apply(this, arguments), this.size = new Size(SmallSwatch.SIZE_PERCENT, SmallSwatch.SIZE_PERCENT), 
            this.position = new Position(0, 0);
        }
        return extend(SmallSwatch, Swatch), SmallSwatch.SIZE_PERCENT = 10, SmallSwatch;
    }
    angular.module("para2016").factory("SmallSwatch", [ "Swatch", "extend", "Size", "Position", SmallSwatchFactory ]);
}(), function() {
    function createTransformObjectFactory() {
        function createTransformObject(value) {
            return {
                transform: value,
    ...
    angular.module("para2016").factory("createTransformObject", createTransformObjectFactory);
}(), function() {
    function TrendGroup2016Factory(TrendGroup, extend, SmallSwatch, relationManager, PositionableElement) {
        function TrendGroup2016() {
            TrendGroup.apply(this, arguments);
        }
        return extend(TrendGroup2016, TrendGroup), TrendGroup2016.GAP_PERCENT = 1.25, TrendGroup2016.ROOM_VERTICAL = 5 * SmallSwatch.SIZE_PERCENT + 4 * TrendGroup2016.GAP_PERCENT, 
        TrendGroup2016.prototype.blurbElement = void 0, relationManager.registerHasOneRelation(TrendGroup2016, "blurbElement", PositionableElement), 
        TrendGroup2016;
    }
    angular.module("para2016").factory("TrendGroup2016", [ "TrendGroup", "extend", "SmallSwatch", "relationManager", "PositionableElement", TrendGroup2016Factory ]);
}(), function() {
    function UrbanJungleTrendGroupFactory(TrendGroup2016, extend, createPopulatedInstances, createPopulatedInstance, SmallSwatch, LargeSwatch, Colour, StockPhoto, RoomPhoto, Position, Size, PositionableElement) {
        function UrbanJungleTrendGroup() {
            this.title = "urban jungle", this.description = "Colours, textures and finishes that inspire the changing biodiversity of earth. Transforming our living spaces into an island of our own in a constantly growing urban environment. ";
            var smallSwatches = createPopulatedInstances(SmallSwatch, [ {
                name: "weathered oak",
                code: "Minwax",
                src: "images/urban_jungle/minwax.jpg",
                isDark: !0,
                position: new Position(0, 0)
            }, {
                name: "stainless steel",
                code: "Krylon",
                src: "images/urban_jungle/krylon.jpg",
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, 0)
            }, {
                name: "heavy metal",
                code: "P5244-73D",
                colour: new Colour(85, 78, 76),
                isDark: !0,
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 0)
            }, {
                name: "great expectations",
                code: "P5022-41",
                colour: new Colour(116, 203, 201),
                position: new Position(3 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 0)
            }, {
                name: "olive grove",
                code: "PF59",
                colour: new Colour(121, 111, 72),
                isDark: !0,
                position: new Position(0, SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)
            }, {
                name: "ripe mango",
                code: "P5063-73",
                colour: new Colour(226, 143, 38),
                position: new Position(SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)
            }, {
                name: "guava",
                code: "PF32",
                colour: new Colour(233, 104, 72),
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)
            } ]), largeSwatches = createPopulatedInstances(LargeSwatch, [ {
                name: "greek oregano",
                code: "PF58",
                colour: new Colour(179, 163, 124),
                position: new Position(0, -0)
            }, {
                name: "ceviche",
                code: "PF51",
                colour: new Colour(208, 185, 97),
                position: new Position(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, -0)
            }, {
                name: "carrara",
                code: "PF53",
                colour: new Colour(217, 211, 206),
                position: new Position(2 * (LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -0)
            } ]);
            this.swatches = [].concat(smallSwatches, largeSwatches), this.stockPhotos = createPopulatedInstances(StockPhoto, [ {
                src: "images/urban_jungle/stock_left.jpg",
                position: new Position(0, 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)),
                size: new Size(LargeSwatch.SIZE_PERCENT, TrendGroup2016.ROOM_VERTICAL - 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }, {
                src: "images/urban_jungle/stock_bottom.jpg",
                position: new Position(3 * (LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), -0),
                size: new Size(100 - 3 * (LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), LargeSwatch.SIZE_PERCENT)
            } ]), this.roomPhotos = createPopulatedInstances(RoomPhoto, [ {
                src: "images/urban_jungle/room.jpg",
                position: new Position(-0, -(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)),
                size: new Size(100 - 2 * (LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), TrendGroup2016.ROOM_VERTICAL)
            } ]), this.blurbElement = createPopulatedInstance(PositionableElement, {
                position: new Position(2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT), 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT)),
                size: new Size(LargeSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT, TrendGroup2016.ROOM_VERTICAL - 2 * (SmallSwatch.SIZE_PERCENT + TrendGroup2016.GAP_PERCENT))
            }), TrendGroup2016.apply(this, arguments);
        }
        return extend(UrbanJungleTrendGroup, TrendGroup2016), UrbanJungleTrendGroup;
    }
    angular.module("para2016").factory("UrbanJungleTrendGroup", [ "TrendGroup2016", "extend", "createPopulatedInstances", "createPopulatedInstance", "SmallSwatch", "LargeSwatch", "Colour", "StockPhoto", "RoomPhoto", "Position", "Size", "PositionableElement", UrbanJungleTrendGroupFactory ]);
}();